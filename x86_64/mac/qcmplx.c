...
void EXPORT qcabs( qcmplx *a, Qfloatp y)
{
	...
	ea = (unsigned int )a->re[1].exponent;
	eb = (unsigned int )a->im[1].exponent;
	if( ((ea - eb) > NBITS) || (eb == 0) )
	{
		qmov( &a->re[0], y );
		y[0].sign = 0;
...
void EXPORT qcsqrt(qcmplx *z,qcmplx *w)
{
	...
	*/
	...
	if( eb > ec )
	    ...
	    qclear( &y.im[0] );
	qtoe( &y.re[0], (unsigned short *) &dx );
	qtoe( &y.im[0], (unsigned short *) &dy );
	/* csqrt( &dz, &dz ); */
	dr = sqrt(dx*dx + dy*dy);
	if( dx > 0 )
	{
	...
	if (dy < 0)
		dr = -dr;
	etoq( (unsigned short *) &dt, &q.re[0] );
	etoq( (unsigned short *) &dr, &q.im[0] );
	/* Fix signs.  */
	q.re[0].sign = 0;
