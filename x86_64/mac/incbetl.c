...
#ifdef __LCC__
long double __declspec(naked) beta_incomplete(long double aa, long double bb,long double x)
{
}
#endif
...
domerr:
	...
	if( (a+b) < MAXGAML && fabsl(y) < MAXLOGL && fabsl(t) < MAXLOGL )
	{
		...
		t /= a;
		t *= w;
		t *= tgammal(a+b) / (tgammal(a) * tgammal(b));
		goto done;
	}
	else
	{
		/* Resort to logarithms.  */
		y += t + lgammal(a+b) - lgammal(a) - lgammal(b);
		y += logl(w/a);
		if( y < MINLOGL )
			t = 0.0L;
		else
...
#ifndef __LCC__
long double beta_incomplete(long double aa, long double bb,long double x)
{
	return incbetl(aa,bb,x);
}
#endif
...
static long double pseriesl(long double a,long double b,long double x )
{
	...
	if( (a+b) < MAXGAML && fabsl(u) < MAXLOGL )
	{
		...
		{
		t = lgammal(a+b) - lgammal(a) - lgammal(b) + u + logl(s);
		if( t < MINLOGL )
			s = 0.0L;
		else
			s = expl(t);
