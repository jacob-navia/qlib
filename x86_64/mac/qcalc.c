...
int take(char *);
void mxit(void);
int bits(QELT *), cmp(Qfloatp, Qfloatp, Qfloatp);
...
getcmd:
	...
	__qclear(acc);	/* Clear the accumulator				*/
		...
		{ /* leading "0" may mean octal or hex radix */
			...
			else
			{
				while( ISOCTAL( *pline ) )
				{
					i = (*pline++ & 0xff) - 060;
					nc = ( nc * 8.0) + i;
					etoq( (unsigned short *) &nc, (Qfloatp)qnc );
				}
				goto numdon;
			}
		}
...
numcvt:
			*pn++ = ' ';
			*pn++ = 0;
			asctoq( number, (Qfloatp)qnc ,NULL);
			/*		sscanf( number, "%le", &nc );*/
		}
		/* output the number	*/
numdon:
		...
		for( i=0; i<NCONST; i++ )
		{
			if( (pvar->attrib & BUSY) == 0 )
				goto confnd;
			if( qcmp( pvar->value, (Qfloatp)qnc) == 0 )
			{
...
int hexbits(Qfloatp u)
{
	...
	pi = (int *)x;
	printf("{%d,0x%08x,",pi[0],pi[1]);
	j = 0;
	for( i=2; i<9; i++ )
	{
		printf( "0x%016llxULL,", x[i] );
		if( savfil )
			fprintf( savfil, "0x%016llxxULL,", x[i] );
		if( ++j > 3 )
		{
			j = 0;
			printf( "\n " );
	...
	if( savfil )
		fprintf( savfil, "\n" );
#if 1
	qtoe64((Qfloatp)u,(unsigned short *)&dd);
	pi = (int *)&dd;
	pll = (long long *)&dd;
	printf("Floating point 0x%llx 0x%x 0x%x (%Lg)\n",pll[0],pi[2],pi[3],dd);
#endif
	return 0;
}
...
int bits(QELT u[] )
{
	...
	qmovz( (Qfloatp)u, (QfloatAccump)x );
	/* Print Q-type value in hex.  */
	j = 0;
	for( i=0; i<NQ; i++ )
	{
