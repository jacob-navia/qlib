...
int qjypn(Qfloatp qnn,Qfloatp x,Qfloatp y)
{
	static Qfloat nfac[1], nm1fac[1], f[1], a[1], psi1[1], psin[1];
	static Qfloat g[1], h[1], jn[1], yn[1], z[1], s[1];
	static Qfloat qn[1], p[1], q[1], t[1];
	static Qfloat tt[1], u[1], odd[1], tlast[1];
	int temp;
	int i, k, n, sign, kpn;
	double dn;
	...
	while( ((int) a[0].exponent - (int) s[0].exponent) > -NBITS/2 );
	...
	*/
	...
	qdiv( qpi, g, yn );		/*s /= PI;*/
	if( sign < 0 )
		yn[0].sign = ~yn[0].sign;		/*return(s*sign);*/
	goto findp;
	/*		jvpq.c
...
hank:
	...
	do
		...
		qmov( t, tlast );
		t[0].sign = temp;
		if( qn[0].exponent > (QELT) (EXPONE + 6) )
			goto done;
		sign = -sign;
	}
