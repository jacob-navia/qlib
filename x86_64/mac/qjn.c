...
void qjn(Qfloatp nn,Qfloatp xx,Qfloatp y)
{
	...
	qmov( nn, n );
	qmov( xx, x );
	__qtoe64( n, (unsigned short *) &dn );
	__qtoe64( x, (unsigned short *) &dx );
	sign = 1;
	an = fabsl( dn );
	if( an == floorl(an) )
	{
		i = an - 16384.0 * floorl( an/16384.0 );
		if( n[0].sign != 0 )
		{
			if( i & 1 )
				sign = -sign;
...
void qhank(Qfloatp n,Qfloatp x,Qfloatp y)
{
	int bt;
...
hank1:
	...
	if(i > NBITS/2
			|| DEBUG )
	{
		__qtoe64( n, (unsigned short *) &dconv );
		printf( "qhank(%.5Le,", dconv );
...
#if DEBUG
	__qtoe64( ans, (unsigned short *) &dconv );
