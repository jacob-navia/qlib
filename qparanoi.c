...
#define FLOAT(x) static Qfloat x[1] = {0}
...
#define LOG(x,y) qflog(x,y)
#define POW(x,y,z) qfpow(x,y,z)
#define SQRT(x,y) qfsqrt(x,y)
...
#define LTOF(i,x) itoq(i,x)

/* Convert FLOAT a to decimal ASCII string with b digits */
#define TOASC(a,b,c,d,e) qtoasc(a,b,c,d,e)
...
long long lngint;	/* intermediate for conversion between int and FLOAT */
...
under2:
	...
	if( cmp( Y2,UfThold) <= 0 )
	{
		...
		printf("Range is too narrow; U1^%lld Underflows.\n", I);
	}
	Milestone = 130;
...
{
	if(N > 0)
		printf("Similar discrepancies have occurred %lld times.\n", N);
}
