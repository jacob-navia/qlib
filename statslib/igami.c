...
long double gamma_incomplete_inv(long double a,long double y0 )
{
	...
	x1 = 0;
	yh = 1.0;
	dithresh = 5.0 * LDBL_EPSILON;

	/* approximation to inverse function */
	d = 1.0/(9.0*a);
	y = ( 1.0 - d - normal_distribution_inv(y0) * sqrt(d) );
	x = a * y * y * y;
...
ihalve:
	...
	if( x == 0.0 )	mtherr( "igami", UNDERFLOW );
