 ...
 * ACCURACY:
*
 * Series summation terminates at NBITS.
*
 */
...
static Qfloat A[NBERCOF];
#define PRINT
#define NLOGS	170
static Qfloat logs[NLOGS];

static int qber_ini = 0;
static int init_bernum(void)
{
	Qfloat den[1], qi[1];
	int i, k;
...
#ifdef PRINT
	k = 0;
	FILE *f=fopen("bern.c","w");
	fprintf(f,"static Qfloat Bern[] = {\n");
	for( i=0; i<NBERCOF; i++ )
	{
		asctoq( bern_str[k++], A+i,NULL );
		asctoq( bern_str[k++], den,NULL );
		qdiv( den, A+i, A+i );
		fprintf(f,"/* %s / %s (b%d)*/\n",bern_str[k-2],bern_str[k-1],k);
		fprintf(f,"{%3d,0x%08x,{0x%016llx,0x%016llx,0x%016llx,0x%016llx,\n\t0x%016llx,0x%016llx,0x%016llx}},\n",
		A[i].sign,A[i].exponent,A[i].mantissa[0],A[i].mantissa[1],A[i].mantissa[2],A[i].mantissa[3],
		A[i].mantissa[4],A[i].mantissa[5],A[i].mantissa[6]);
	}
...
void qzeta3(Qfloatp x,Qfloatp y)
{
	int s = notwo;
	notwo = 1;
	qzetac( x, y );
...
void qzetac(Qfloatp x,Qfloatp y)
{
	...
	*/
	...
	do {
		ia += 1;		/* a += 1.0 */
		/* b = power( a, -x ) */
		qmov( &logs[ia-2], t );	/* qflog( a, t ) */
		qmul( x, t, t );
		qfexp( t, b );
		qdiv( b, qone, b );
		qadd( s, b, s );	/* s += b */
		if( ((int) s[0].exponent - (int) b[0].exponent) > NBITS)
			goto done;
	}
	...
	for( i=0; i<NBERCOF; i++ ) {
		...
		qmul( a, t, a );
		qdiv( w, b, b );	/* b /= w */
		qmul( &A[i], b, t );	/* t = a*b*A[i] */
		qmul( a, t, t );
		qadd( s, t, s );	/* s = s + t */
		if( t[0].exponent >= min )
			break;
		min = t[0].exponent;
		conv = (int) s[0].exponent - (int) t[0].exponent;
		if( conv > NBITS )
			goto done;
		qadd( qone, k, k );	/* k += 1.0 */
		qadd( x, k, t );	/* a *= x + k */
