...
static Qfloat t2[1];
static Qfloat tlg[1];
static Qfloat qspi[1];
static int qsflg = 0;
#endif
int qkne(Qfloatp integer, Qfloat x[],Qfloat y[] )
{
	Qfloat k[1];
	Qfloat kf[1];
	Qfloat nk1f[1];
	...
	while( ((int) s[0].exponent -  t[0].exponent) < NBITS/2 ); /* fabs(t/s) > MACHEP ); */
	...
	while( ((int) s[0].exponent - t[0].exponent) < NBITS/2 );
