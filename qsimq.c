...
int simq(Qfloat qA[1],Qfloat qB[1],Qfloat qX[],int n,int flag,int IPS[])
{
	...
	{
		IPS[i] = i;
		qclear( (Qfloat *)rownrm );
		for( j=0; j<n; j++ )
		{
			qmov( (Qfloat *)&A[NQ*ij], (Qfloat *)q );	/* q = abs( A[ij] ) */
			q[0].sign = 0;
			if( qcmp( rownrm, q ) < 0 )	/* rownrm < q */
				qmov( q, rownrm );	/* rownrm = q */
			ij += 1;
		}
		if( rownrm[0].exponent < 2  )
		{
			printf("SIMQ ROWNRM=0");
			return(1);
		}
		qdiv( rownrm, qone, (Qfloatp)&X[NQ*i] );	/* X[i] = 1.0/rownrm */
	}
	...
	for( k=0; k<nm1; k++ )
	{
		...
		for( i=k; i<n; i++ )
		{
			...
			ipk = n*ip + k;
			/* size = abs( A[ipk] ) * X[ip] */
			qmov( (Qfloatp)&A[NQ*ipk], size );
			size[0].sign = 0;
			qmul( size, (Qfloatp)&X[NQ*ip], size );
		...
		kp = IPS[k];
		kpk = n*kp + k;
		qmov( (Qfloatp)&A[NQ*kpk], pivot );	/* pivot = A[kpk] */
		kp1 = k+1;
		for( i=kp1; i<n; i++ )
		{
			ip = IPS[i];
			ipk = n*ip + k;
			ptr = (Qfloatp)&A[NQ*ipk];
			...
			for( j=kp1; j<n; j++ )
			{
				...
				ptr = (Qfloatp)&A[NQ*ipj];
				qmul( (Qfloatp)&A[NQ*(nkp+j)], em, q );
				qadd( ptr, q, ptr );
			}
		}
	}
...
solve:
	ip = IPS[0];
	qmov( (Qfloatp)&B[NQ*ip],(Qfloatp) &X[0] );	/* X[0] = B[ip] */
	for( i=1; i<n; i++ )
	{
		ip = IPS[i];
		ipj = n * ip;
		...
		for( j=0; j<i; j++ )
		{
			qmul( (Qfloatp)&A[NQ*ipj], (Qfloatp)&X[NQ*j], q ); /* sum += A[ipj] * X[j] */
			qadd( sum, q, sum );
			++ipj;
		}
		qsub( sum, (Qfloatp)&B[NQ*ip], (Qfloatp)&X[NQ*i] ); /* X[i] = B[ip] - sum */
	}

	ipn = n * IPS[n-1] + n - 1;
	ptr = (Qfloatp)&X[NQ*(n-1)];
	qdiv( (Qfloatp)&A[NQ*ipn], ptr, ptr );	/* X[n-1] = X[n-1]/A[ipn] */
	...
	for( iback=1; iback<n; iback++ )
	{
		...
		{
			/* sum += A[nip+j] * X[j] */
			qmul( (Qfloatp)&X[NQ*j], (Qfloatp)&A[NQ*(nip+j)], q );
			qadd( sum, q, sum );
		}
		ptr = (Qfloatp)&X[NQ*i];
		qsub( sum, ptr, q );	/* X[i] = (X[i] - sum)/A[nip+i] */
		qdiv( (Qfloatp)&A[NQ*(nip+i)], q, ptr );
	}
#endif
	return(0);
}
