...
#	}
#}
# Registters used: x0, x1, x2 x3, x4, x5, x6, x7, x8, x9
	ldp x9,x8,[x0,64]
	mov x6,64
	sub x5,x6,x1
	lsr x4,x8,x5
...
# in both to maximize parallelism
	...
	.balign 8
# -----------------------------------------------------------------------------
#  void _shiftdownn(QfloatAccump x,int n)
#  {
#  	QELT newbyt, oldbyt;
#  	int i;
#  
...
# -----------------------------------------------------------------------------
	.globl	_shdn1
# void _shdn1(QfloatAccump x)
# This function should never use x14,x11, since it is called from mdnorm
# Registers used: x0 to x8
_shdn1:
    ldp x2,x3,[x0,8]
    ldp x4,x5,[x0,24]
    extr x6,xzr,x2,1
...
# x0,x1 contain the 128 bit number (high,low)
# x1 is reused in the next call
# x2 contains the 64 bit number
# Result (64 bit) of division is in x6
# Modulo in x1
# Registers used: x0, x1, x2, x3, x5, x6, x7, x8, x9, x10, x11, x12
# 
	.balign 8
...
#		 if( exponent(r) == 0 ) return 0;
#		 if (signof(r) == 0) return 1;
#		 return -1;
#
#	}
	ldr	w4, [x0, 4]
